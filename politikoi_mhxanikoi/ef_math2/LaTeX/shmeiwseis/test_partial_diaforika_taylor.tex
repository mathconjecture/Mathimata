...
\geometry{left=15mm,right=15mm,top=30.00mm,bottom=32.00mm,footskip=24.16mm,headsep=24.16mm}
...
\begin{rem}
  Γενικότερα, για συναρτήσεις πολλών μεταβλητών, 
  η \textcolor{Col1}{μερική παράγωγος της $f$ ως προς $ x_{i} $} 
...
\item {}
  \begin{align*}
    f_{x}&=2xyz+ y\sin{(xy)}(xy)_{x} = 2xyz+y^{2} \sin{(xy)} \\
    f_{y}&=x^{2}z - \cos{(xy)} + y\sin{(xy)}(xy)_{y}=x^{2}z - \cos{(xy)} + 
    xy \sin{(xy)} \\ 
    f_{z}&=x^{2}y
  \end{align*}
\end{solution}
